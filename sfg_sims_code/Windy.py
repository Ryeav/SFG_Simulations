...
def exec(w0comb,t_delay_range,L,T):
     cpus = int(max_cpus /1)
     #w0comb = [40e-6,50e-6] #sig, pump
     #td_range = np.linspace(-1.2,0.5,acc)*1e-12#-L/(2*vg_p)
     params = [[*w0comb,tdelay,L,T] for tdelay in t_delay_range]
     with Pool(cpus) as pool:
            with tqdm(
                total=len(t_delay_range), desc="Calculating E_sfg", ncols=100
            ) as bar:
                resultlin = []
...
if __name__ == "__main__":
        ...
        Te,P_fwhm,S_fwhm = outside_params()
        acc = int(96)
        w0comb = [40e-6,50e-6] #sig, pump
        L = 3e-3
        Temp = 85
        params = const_params(0,w0comb,L,Temp)
        t_delay_range = np.linspace(-1.2,0.5,acc)*1e-12#-L/(2*vg_p)
        td_r = acc
        xtr = int(14)
        #for k in range(acc):
        #     if k > ((acc)/2-xtr) and k<(acc/2):
        #          t_delay_range = np.insert(t_delay_range,k+1,t_delay_range[k]+(t_delay_range[k+1]-t_delay_range[k])/2)
        #     if k > ((acc+xtr-1)/2) and k<((acc+xtr-1)/2)+3:
        #          t_delay_range = np.insert(t_delay_range,k+1,t_delay_range[k]+(t_delay_range[k+1]-t_delay_range[k])/2)
                  
        
        print(t_delay_range,(len(t_delay_range)))          
        d_eff,Delta_k, vg_p,vg_s, vg_sfg,n_p,n_s,n_sfg ,wl_p,wl_s,wl_sfg, e0,c0,pi,r_points,t_points,dr,dt,r_range,t_range,gvd_p,gvd_s,gvd_sfg=params
        max_step = np.pi/wl_s*n_s*dr**2*1e-2
        num_slice =  math.ceil(2e-6/max_step)
        print(f'number of slices: {num_slice}')
        r_range,t_range, L = Grid(t_delay_range[0],w0comb,L,48)
        print_params(Temp)
        dr = r_range[1]-r_range[0]
        dt = t_range[1]-t_range[0]
        print(dr)
        ...
        results = exec(w0comb,t_delay_range,L,Temp)
        print(np.max(results))
        np.save(rf'C:\\Users\\{user}\\SFG Simulation\\Sim_Data\\DTLYS_TDelay_Temp{Temp}_crylen_{int(L*1e3)}_acc{int(acc)}_fwhmsp{round(S_fwhm*1e15),round(P_fwhm*1e15)}fs_tdrange{float(t_delay_range[0]),float(t_delay_range[-1])}_w0comb{w0comb}', results)
        
        resav =[(res/np.average(results))for res in results]
        E_ph_sfg = const.Planck* c0/wl_sfg
        Signal_rep_rate = 7.64e7
        results = [(res  *(1/(E_ph_sfg))*Signal_rep_rate*0.4*0.9*1e-6) for res in results]            
        plt.plot(t_delay_range*1e12, results, label = f'Temp. Profile, crystal-len.= {L*1e3} mm',color = 'red')
        
        plt.xlabel(fr'Time Delay [ps]')
        plt.ylabel('SFG Photon Counts per Second [kcps]')
        max_res_i =np.argmax(results)
        hm_r1 = [abs(res-np.max(results)/2) for res in results[:max_res_i]]
        hm_r2 = [abs(res-np.max(results)/2) for res in results[max_res_i:]]
        hm1 = t_delay_range[np.argmin(hm_r1)]
        hm2 = t_delay_range[np.argmin(hm_r2)]
        fwhm = abs(hm2- hm1)
        def interpolate_half_max(x, y, half_max):
           for i in range(1, len(y)):
             if (y[i-1] - half_max) * (y[i] - half_max) < 0:  # Sign change
            # Linear interpolation
              return x[i-1] + (half_max - y[i-1]) * (x[i] - x[i-1]) / (y[i] - y[i-1])
           return None

        peak_val = np.max(results)
        max_i = np.argmax(results)

        hm1 = interpolate_half_max(t_delay_range[:max_i+1], results[:max_i+1], peak_val / 2)
        hm2 = interpolate_half_max(t_delay_range[max_i:], results[max_i:], peak_val / 2)
        hm2 += t_delay_range[max_i] if hm2 is not None else 0  # adjust offset

        fwhm = abs(hm2 - hm1)
        #plt.title(f'FWHM: {round(fwhm *1e15,1)} fs')
        plt.legend()
        print(t_delay_range[np.argmax(results)]*1e12)
        print()
        
        plt.savefig(fr"C:\\Users\\{user}\\DTLYS_acc{acc}_Temp{Temp}_Len{L*1e3}_FWHM(p,s){P_fwhm,S_fwhm}_w0Comb(s,p){w0comb}.png")
