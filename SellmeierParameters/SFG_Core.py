...
def Grid(td,w0comb,L,T,wl_p,wl_s):
     ...
     wl_p_m = wl_p *1e-6
     wl_s_m = wl_s *1e-6
     if np.max([BR(w0_s,wl_s_m,-L/2 ),BR(w0_p,wl_p_m,-L/2 )])*3< 8 * w0_max:
     ...
     r_rlin = np.linspace(r_start,r_end,r_points)
...
###################################################################################################################################################################################################################    
    ...
    for e,t in enumerate(t_range):
        for n, r in enumerate(r_range):
            ...
            P0_signal[e] += 2*np.pi*abs(A0_signal[n][e])**2 *dr *r* n_s*e0 *c0
...
def exec(w0combs,td, L,Temp, td_range = None , L_range = None, Temp_range = None, Vox = False, wl_p = 0.82, wl_s = 1.52, PDL = 8.55e-6,Pump_Power_avg = 3e-1,Pump_rep_rate=7.64e7, Nova = False, CPU_cap = max_cpus):
     ...
     if len(w0combs) != 1: 
          ...
          if err_sum > 1 : 
               ...
               raise ValueError('The simulation framework requires one parameter range as input.')
