...
def GVD(wave,T):
         ...
         dlambda_fn = (-(B + b_t )/(wave**2 -(C + c_t)**2)**2*2*wave- E/(wave**2 - F**2)**2*2*wave+2*D*wave)
         dn_dlambda = 1/2 * (Sellmeier(wave,T))**(-1) * dlambda_fn
         dw_lambda = -2*np.pi*const.c/w**2
         dw2_lambda = 4*np.pi*const.c/w**3
         dlambda2_fn = -2*((B+b_t)/(wave**2 +(C+c_t)**2)**2+E/(wave**2-F**2)**2-D)-2*wave*(-2*(B+b_t)/(wave**2 + (C+c_t)**2)**3*2*wave-2*E/(wave**2-F**2)**3*2*wave)#(2*(B + b_t )/(wave**2 -(C + c_t)**2)**3*2*wave  -(B + b_t )/(wave**2 -(C + c_t)**2)**2*2  + 2*  E/(wave**2 - F**2)**3*2*wave- E/(wave**2 - F**2)**2*2+2*D) 
         d2n_dlambda2 = -1/4 * Sellmeier(wave,T)**(-3) *1e-12* dlambda_fn**2 + 1/2 * (Sellmeier(wave,T))**(-1) * dlambda2_fn *1e-12
         
         
         return ((wave)**3/(2*np.pi*const.c**2))*d2n_dlambda2#1/const.c * (2* dn_dlambda * dw_lambda + w* (d2n_dlambda2*dw_lambda**2 + dn_dlambda*dw2_lambda)) 
