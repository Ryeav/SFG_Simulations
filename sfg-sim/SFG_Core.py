...
def tenv_gauss(FWHM,tdelay,t):
     ...
     return  np.exp(-(t-tdelay)**2 / ( 4* (sigma**2) ) )
...
#------------------------------------------------------------------------#
@njit(cache = True)
def nonlin_op(Delta_k, A,z,r_points,t_points,PDL):
    Lambda = PDL#8.55e-6 
    pi = 3.14159265359
    #Delta_k = 3.2 / 2e-3 
...
#@njit
def Grid(td,w0comb,L,T,wl_p,wl_s):
     ''' Setup function for the radius and time grid'''
     wl_sfg = (wl_s*wl_p)/((wl_p + wl_s))
     waves = [wl_p,wl_s,wl_sfg]
     ...
     wl_p_m = wl_p *1e-6
     wl_s_m = wl_s *1e-6
     r_end =np.max([BR(w0_s,wl_s_m,-L/2 ),BR(w0_p,wl_p_m,-L/2 )])*3
     r_start =1e-8
     dr = w0_min/10
     r_points =2*int((r_end-r_start)/dr )//2+1
     r_r = np.linspace(r_start,r_end,r_points)
     gd = (1/vg_p-1/vg_s)
     enve_p = 5 * P_fwhm
     enve_s = 5 * S_fwhm
     sigma_sum = P_fwhm + S_fwhm
     pad = (9 + 2*(int(L/1e-3)-2))* sigma_sum 
...
def const_params(td,w0comb,L,T,wl_p,wl_s, PDL):
     '''
     ...
     d_eff = 5e-12
     waves = [wl_p,wl_s,wl_sfg]
     wl_p_m,wl_s_m,wl_sfg_m = wl_p *1e-6,wl_s*1e-6,wl_sfg*1e-6
     ...
     r_range,t_range,L = Grid(td,w0comb,L,T,wl_p,wl_s)
     ...
     return d_eff,Delta_k, vg_p,vg_s, vg_sfg,n_p,n_s,n_sfg ,wl_p_m,wl_s_m,wl_sfg_m, e0,c0,pi,r_points,t_points,dr,dt,r_range,t_range,gvd_p,gvd_s,gvd_sfg,PDL
...
def DFsys(z,A,params):
     d_eff,Delta_k, vg_p,vg_s, vg_sfg,n_p,n_s,n_sfg ,wl_p_m,wl_s_m,wl_sfg_m, e0,c0,pi,r_points,t_points,dr,dt,r_range,t_range,gvd_p,gvd_s,gvd_sfg,PDL=params 
     ...
     kpump = 2 * pi / wl_p_m# *n_p
     ksig = 2 * pi / wl_s_m #*n_s
     ksfg = 2 * pi / wl_sfg_m #* n_sfg
     wsig = 2*pi*c0/wl_s_m
     wpump = 2*pi*c0/wl_p_m
     wsfg = 2*pi*c0/wl_sfg_m
     pre_pump = 2*wpump/(n_p*c0)
     pre_sig = 2*wsig/(n_s*c0)
     pre_sfg = 2*wsfg/(n_sfg*c0)
     
     nonlin_pump,nonlin_sig,nonlin_SFG = nonlin_op(Delta_k,A,z, r_points,t_points,PDL)
     GD_pump = 0
     ...
     dA_pump =   GD_pump   +  1j*d_eff*pre_pump*nonlin_pump  + 1*1j/(2*kpump*n_p)  *d_r(A_pump,dr,r_range)   + 1j*GVD_pump          
...
def init_grid(z0,r_range,t_range,tdelay,L,w0comb,T,wl_p,wl_s,P0_avg,Pump_rep_rate,Nova,PDL):
    w0_s,w0_p = w0comb
    params = const_params(tdelay,w0comb,L,T,wl_p,wl_s, PDL)
    r_range,t_range, L = Grid(tdelay,w0comb,L,T,wl_p,wl_s)
    Temp,P_fwhm,S_fwhm = outside_params()
    
    d_eff,Delta_k, vg_p,vg_s, vg_sfg,n_p,n_s,n_sfg ,wl_p_m,wl_s_m,wl_sfg_m, e0,c0,pi,r_points,t_points,dr,dt,r_range,t_range,gvd_p,gvd_s,gvd_sfg,PDL = params
    z_r = np.max([pi*w0_p**2*n_p/wl_p,pi*w0_s**2*n_s/wl_s])
    #pulse_length = t_range[-1]-t_range[0]   
    E_photon_s = const.Planck * c0/wl_s
    avg_power_pump = P0_avg
    pump_rep_rate = Pump_rep_rate 
    intensity_sum_s = np.sum([tenv_gauss(S_fwhm,-tdelay,t)**2 for t in t_range])*dt
    P0_s = E_photon_s/ intensity_sum_s
    dmg_limit = 1e3 # Damage Treshold of Crystal
    ...
    if Nova == False:
        ...
        if pump_sech == 1:
            intensity_sum_p = np.sum([tenv_sech(P_fwhm,0,t)**2 for t in t_range])*dt
            P0_p = (1/2 * pi * dmg_limit* w0_p**2) / intensity_sum_p
        else:
            intensity_sum_p = np.sum([tenv_gauss(P_fwhm,0,t)**2 for t in t_range])*dt
            P0_p =  (1/2 * pi * dmg_limit* w0_p**2) / intensity_sum_p
    ...
    AMP0_p = np.sqrt(P0_p/(n_p*np.pi*e0 *c0*w0_p**2))      
            #  |  set up r x t l and calculate initial conditions + boundary conditions dr|r==0 = 0, A_j|r== rmax = 0
            #  v  initial field amplitudes
    KSig = 2*pi/wl_s
    KPump = 2*pi/wl_p

    A_SFG = np.array([[complex(0,0) for _ in t_range] for _ in r_range])
...
def SomewhereIBelong(args):
    w0_s,w0_p,tdelay,L,T,wl_p,wl_s,PDL,P0_avg,Pump_rep_rate,Nova = args
    w0comb = [w0_s,w0_p] 
    params = const_params(tdelay,w0comb,L,T,wl_p,wl_s, PDL)
    r_range,t_range, L = Grid(tdelay,w0comb,L,T,wl_p,wl_s)
    Temp,P_fwhm,S_fwhm = outside_params()
    ...
    z0 = 0
    Y0= init_grid(z0-L/2,*Grid(tdelay,w0comb,L,T,wl_p,wl_s)[:-1],tdelay,L,w0comb,T,wl_p,wl_s,P0_avg,Pump_rep_rate,Nova,PDL)

    ze = L
    dr_avg = dr#5e-7
    max_step =np.min([pi/wl_s *n_s *dr_avg**2,pi/wl_p *n_p *dr_avg**2])
    ...
    Y01 = init_grid(z0-L/2,*Grid(tdelay,w0comb,L,T)[:-1],tdelay,L,w0comb,T,wl_p,wl_s,P0_avg,Pump_rep_rate,Nova,PDL)
    ...
    for e,t in enumerate(t_range):
        for n, r in enumerate(r_range):
            P_signal[e] += 2*np.pi*np.abs(A_signal[n][e])**2 *dr *r * n_s*e0 *c0
            P0_signal[e] += 2*np.pi*np.abs(A0_signal[n][e])**2 *dr *r* n_s*e0 *c0
        E_signal += P_signal[e]*dt
        E0_signal += P0_signal[e]*dt  
    

    return 1-E_signal/E0_signal    



def exec(w0combs,td, L,Temp, td_range = None , L_range = None, Temp_range = None, Vox = False, wl_p = 0.82, wl_s = 1.52, PDL = 8.55e-6,Pump_Power_avg = 3e-1,Pump_rep_rate=7.64e7, Nova = False, CPU_cap = max_cpus):
     '''
     Enter Wavelengths --> wl_p and wl_s in micrometers and Periodic Poling Distance PDL in meters. 
     '''
     cpus = int(CPU_cap)
     if w0combs.ndim != 1: 
          num_combs = len(w0combs)
          acc = int(np.sqrt(len(w0combs)))          
          params = [[*w0comb,td, L , Temp,wl_p,wl_s,PDL,Pump_Power_avg,Pump_rep_rate,Nova] for w0comb in w0combs]
     else: 
          ranges_r = [td_range,L_range,Temp_range] 
          err_sum = np.sum([arg is not None for arg in ranges_r])
          if err_sum > 1 : 
               raise ValueError('The simulation framework can only handle one parameter range at once.')  
          elif err_sum == 0:
               raise ValueError('The simulation framework requires exactly one parameter range as input.')
          
          elif td_range != None:
                    params = [[*w0combs,tdelay,L,Temp,wl_p,wl_s,PDL,Pump_Power_avg,Pump_rep_rate,Nova] for tdelay in td_range]  
          elif L_range != None:
                    params = [[*w0combs,td * l/2,l, Temp,wl_p,wl_s ,PDL,Pump_Power_avg,Pump_rep_rate,Nova] for l in L_range]
          elif Temp_range != None:
                    params = [[*w0combs,td , L, T ,wl_p,wl_s,PDL,Pump_Power_avg,Pump_rep_rate,Nova] for T in Temp_range]
