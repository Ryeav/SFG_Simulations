...
from SellmeierParameters.Sellmeier_GroupVelocity import get_params,print_params, GroupVelocity
import datetime
...
def init_grid(z0,r_range,t_range,tdelay,L,w0comb,T):
    ...
    if pump_sech == 1:
     ...
     P0_p = (1/2 * pi * 1e3* w0_p**2) / intensity_sum_p
    else:
...
###################################################################################################################################################################################################################
    ...
    Y01 = init_grid(-L/2,*Grid(tdelay,w0comb,L,T)[:-1],tdelay,L,w0comb,T)
...
###################################################################################################################################################################################################################    
    ...
    #P_max = np.max(P_signal)    
    

    return  1- E_signal/E0_signal #,[A_signal,A_pump,A_SFG], [A_s_prog, A_p_prog, A_sfg_prog]    
...
if __name__ == "__main__":
        ...
        acc = 16
        w0_p_list = np.linspace(5,25,acc)*1e-6
        w0_s = 15e-6
        w0_s_r = [5e-6,7.5e-6,10e-6,15e-6,20e-6]
        for w0_s in w0_s_r[:1]:
          w0combs = [[w0_s,w0_p] for w0_p in w0_p_list] #sig, pump
          L = 2e-3
          Temp = 60
          params = const_params(0,w0combs[0],L,Temp)
          ...
          r_range,t_range, L = Grid(t_delay_range[0],w0combs[0],L,48)
          print_params(Temp)
          dr = r_range[1]-r_range[0]
          dt = t_range[1]-t_range[0]
          print(dr)
          ...
          results = exec(w0combs,0,L,Temp)
          print(np.max(results))
          np.save(rf'C:\\Users\\{user}\\SFG Simulation\\Sim_Data\\Nova860_tempdiff_Temp{Temp}_crylen_{int(L*1e3)}_acc{int(acc)}_w0_s{round(1e6*w0_s,1)}', results)
        #np.save(rf'C:\\Users\\hys.labo38\\SFG Simulation\\Sim_Data\\TDelay_Temp{Temp}_crylen_{int(L*1e3)}_acc{acc}_tdrange{t_delay_range[0],t_delay_range[-1]}', results)  
        resav =[(res/np.average(results))for res in results]
        E_ph_sfg = const.Planck* c0/wl_sfg
        Signal_rep_rate = 7.64e7
        #results = [(res  *(1/(E_ph_sfg))*Signal_rep_rate*0.4*0.9) for res in results]            
        plt.plot(w0_p_list*1e6, results, label = r'$w_{0,s}$='+f'{round(w0_s*1e6,1)}'+r' $\mu$m',color = 'red')
        
        plt.xlabel(r'$w_{0,p}\, [\mu$m]')
        plt.ylabel('SFG efficiency')
        plt.legend()
        #print(w0_p_list[np.argmax(results)]*1e6)
        #print()
        
        #plt.savefig(fr"C:\\Users\\{user}\\Nova855_acc{acc}_Temp{Temp}_Len{L*1e3}_FWHM(p,s){P_fwhm,S_fwhm}_w0Comb(s,p){w0combs[0]}.png")
