...
def Grid(td,w0comb,L,T):
     ...
     ze = L/2 + z_r
     #r_end = 8 * w0_max
     r_end = np.max([BR(w0_s,wl_s,-L/2 ),BR(w0_p,wl_p,-L/2 )])*3#
     r_start =1e-8
...
if __name__ == "__main__":
        ...
        Temp,P_fwhm,S_fwhm = outside_params()
        acc =5
        w0p_list = np.linspace(5,10, acc) *1e-6  # Range of w0_p values
        w0s_list = np.linspace(5,10 , acc) *1e-6 # Range of w0_s values
        ...
        Optimum = str(r"Max. Power with:  $\omega_{0,\,signal}$ = " + fr" {round(w0s_list[xmax]*1e6,2)} $\mu$m, and "+ r"$\omega_{0,\,pump}$ = " + fr"{round(w0p_list[ymax]*1e6,2)}  $\mu$m")
